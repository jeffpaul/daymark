...
- Every Timeline item now shows its own site icon (a Mark's own avatar, or a subscribed site's icon) as its own leading element, separate from the item's thumbnail. Tapping it is a single click straight to the same Source filter Search already offers — no menu, no separate "visit the site" option.
