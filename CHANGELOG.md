...
- Commenting on a subscribed post whose site declines anonymous comments through its own REST API showed that site's raw rejection text ("Sorry, you must be logged in to comment"), which read as a Daymark login problem rather than what it actually is — a default WordPress restriction on the *destination* site. Now shows a clear explanation and no longer implies more submitted info (name/email/site) would help, since the site's API rejects the request before ever reading it. ([#351](https://github.com/jeffpaul/daymark/issues/351))
