...
- A Draft's own card no longer shows a "Draft" chip — Home's Drafts row and Me's own Drafts list already group it under a "Drafts" section heading, so the chip only ever repeated what that heading already said. ([#405](https://github.com/jeffpaul/daymark/issues/405))
- A Mark with Featured Content set (audio or video, for now) now shows it on the Timeline card — previously the card showed nothing at all for a Note or Checkin Mark whose only visual content was its Featured Content. ([#409](https://github.com/jeffpaul/daymark/issues/409))
