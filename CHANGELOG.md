...
- A new **Featured Content** panel in the block editor sidebar, alongside Featured Image, for any post type that already shows one (not just Marks): set an audio or video file — from your media library, or a pasted URL (YouTube, Vimeo, a podcast episode link) with a live preview — as a post's featured content. Once set, it's shown in place of your Featured Image everywhere your theme already renders one, with no theme changes needed; a theme that wants it to render somewhere different instead can opt out and use the new `daymark_the_featured_content()`/`daymark_has_featured_content()`/`daymark_get_featured_content()` template tags directly. Gallery, quote, and a link-format-specific link field are planned as follow-up phases of the same feature. ([#401](https://github.com/jeffpaul/daymark/issues/401))
