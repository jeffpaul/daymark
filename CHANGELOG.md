...
- Commenting on a subscribed post whose site declines anonymous comments through its own REST API (or where nothing deliverable could be discovered there at all) showed that site's raw rejection text ("Sorry, you must be logged in to comment"), which read as a Daymark login problem rather than what it actually is — a default WordPress restriction on the *destination* site. Now shows a clear explanation, plus a link straight to the original post's own comment form so you can finish leaving your comment there directly. ([#351](https://github.com/jeffpaul/daymark/issues/351))
