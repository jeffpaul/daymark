...
- The Posts list in wp-admin now has a sortable "Format" column, right after Title, showing each post's post format (Standard, Aside, Image, etc.) — click a non-Standard format to filter the list the same way WordPress core's own "All formats" dropdown already does, or click the column header to sort by format.
