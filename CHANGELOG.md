...
- The Posts list in wp-admin now shows a small icon next to a post's title indicating its post format (Aside, Image, Video, etc. — a Standard post shows no icon); WordPress core's own "All formats" dropdown, which already appears on this screen once any post uses a non-Standard format, remains the way to filter by format.
