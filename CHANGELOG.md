...
- Every Timeline item now shows its own site icon (a Mark's own avatar, or a subscribed site's icon) as its own leading element, separate from the item's thumbnail. Tapping it is a single click straight to the same Source filter Search already offers — no menu, no separate "visit the site" option. A Note Mark with no real photo no longer shows a second, type-initial icon next to it — that placeholder is gone, so title and caption text run further before wrapping. A subscription post's own thumbnail slot still always shows something, though: its featured image when it has one, otherwise the subscription's own site icon — a rich-media post (video/audio/gallery) also carries a small format badge over its thumbnail so what a tap leads to is clear before opening it. Either image degrades gracefully to a plain letter glyph if it fails to load (a subscription's site icon is sometimes an unverified guess at the site's `/favicon.ico`, which doesn't always resolve).
