...
- Full offline PWA mode (manifest + conservative service worker only). The composer's autosave (see the architectural decisions table) does not cross this line — it still requires connectivity, exactly like the manual "Save as Draft" it reuses; only surviving genuine offline use is out of scope here, tracked separately in [issue #121](https://github.com/jeffpaul/daymark/issues/121).
- Push notifications
