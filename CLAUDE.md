...
| Offline-first creation (issue #121) | "Creating while offline. Publish later. Users shouldn't care." Scoped to a composer session already open when connectivity drops (or that never had it) — not a cold load of `/daymark` itself with zero connectivity, which stays out of scope (see the non-goals note below and the roadmap's "Offline app-shell load (revisit)" item). `submitOrQueue()` in `assets/app.js` tries the real request first; on a connectivity-shaped failure (`navigator.onLine` false, or `fetch()` itself throwing the `TypeError` browsers use for a request that never reached a server — as opposed to a well-formed HTTP error response, which is rethrown so real server/validation errors still surface normally) it stores the Mark whole — including picked media, as real Blobs, which IndexedDB supports natively — in a `daymark-offline` IndexedDB database instead. `flushOfflineQueue()` replays every queued item through the exact same `POST /marks`/`PUT /marks/{id}` endpoints on the `online` event and once at boot, so the server never sees a different code path for offline-originated work; an item currently open in the composer is skipped by the background flush (its own autosave/publish handles it) to avoid racing in-memory edits newer than the last IndexedDB write. A queued Publish/Save as Draft shows the same success screen as the online path ("Saved offline — will publish/sync automatically"), never an error; a Home "Pending" section (`renderPendingItem()`/`refreshPendingSection()`) lists what's still queued, and `openPendingMark()` resumes one in the composer, rehydrating picked files from their stored Blobs. `buildMarkPayload()` (a plain, structured-cloneable object) is the one field-mapping shared by a live request, an autosave, and a queued replay; `payloadToFormData()` is the one place that turns it into the multipart body all three send. | Directly the requested product concept, scoped narrowly on purpose: "resilient drafting" (per issue #121's own framing) rather than "full offline app". Reuses the exact publisher/REST code path online creation already uses — no server changes at all — so offline-originated content is indistinguishable from online-originated content the moment it lands. The `online` event (not the Background Sync API) is the sync trigger specifically because Safari/iOS has zero Background Sync support, and this is a phone-first PWA that documents iOS Safari as a baseline target. |
