...
class Moment_AI_Assist {
	...
	private function generate_suggestion_bundle( array $context ): ?array {
		...
		$prompt = 'Suggest a title, caption, alt text, and 3-5 topic tags for this personal blog moment. '
			...
			. ' Respond as JSON with keys "title" (a short headline, max 60 chars), "caption" (max 200 chars), "alt_text" (max 125 chars, empty string if no media), and "tags" (array of lowercase strings).';
		...
		return array(
			'title'          => '' !== $title ? $title : $this->mock_title( $context ),
	 ...
	 *
	 * Fallback when no AI provider is configured. The real path is the
	 * structured "title" field of generate_suggestion_bundle(). A title is a
	 * short headline (max ~60 chars), distinct from the longer caption body.
	 *
	 * @since 0.5.0
	 *
	 * @param array $context Normalized context.
	...
	private function mock_title( array $context ): string {
		if ( '' !== $context['text'] ) {
			return trim( wp_html_excerpt( $context['text'], 60, '…' ) );
		}

		$titles = array(
			'image'   => 'A Captured Moment',
			'gallery' => 'A Gallery of Moments',
			'video'   => 'A Moment in Motion',
			'audio'   => 'A Moment in Sound',
			'podcast' => 'A New Episode',
			'note'    => 'A Note From Today',
			'mixed'   => 'A Mixed-Media Moment',
		);

		return $titles[ $context['type'] ] ?? $titles['note'];
	}
